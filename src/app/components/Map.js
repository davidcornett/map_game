...
const Map = () => {
  ...
  const [isBuilding, setIsBuilding] = useState(false); // loading status while country building
  ...
  */
  ...
  const handleBuildClick = async () => {
    ...
    try {
      // Wait for the async work to finish so we know when to stop showing the overlay
      await getCountry(Array.from(selectedCounties));

      if (mode === 'sandbox') {
        await fetchNationalParkData(Array.from(selectedCounties));
      }

      setCountryName('');
      setDisplayName('');
    } catch (e) {
      // getCountry already sets validation messages, but this guarantees we don't swallow anything
      console.error(e);
    } finally {
      // ✓ always stop spinner, even if errors occur
      setIsBuilding(false);
    }
  };
...
{/* LOADING STATUS ------------------------------------------------------------------------------*/}
<LoadingOverlay show={isBuilding} message="Building your country…" />
